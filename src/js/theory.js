...
// 	var maxInScale=-999, maxRes=[];
// 	var totalDuration=0;
// 	for (var i=0; i<notes.length; i++) totalDuration+=notes[i].d;
// 	for (var k=0; k<12; k++) {
// 		var ns=copyObj(notes);
// 		for (var n=0; n<ns.length; n++) ns[n].y= (ns[n].y+9+k) % 12;
// 		for (var s=0; s<scaleDict.length; s++) 
// 		if (s!=91)  // excluding chromatic
// //		if (s!=91 && s!=88)  // excluding chromatic & composite blues
// 		{	var inScaleCount=0, inScaleDuration=0, outScaleCount=0;
// 			for (var n=0; n<ns.length; n++){
// 				if (scaleDict[s].mask[ns[n].y]=='1') {
// 					inScaleCount++;
// 					inScaleDuration+=ns[n].d;
// 				} else outScaleCount++;
// 			};
// 			if (inScaleDuration / totalDuration > maxInScale) 
// 				maxInScale = inScaleDuration / totalDuration;
// 		}
// 	};
// 	for (var k=0; k<12; k++) {
// 		var ns=copyObj(notes);
// //		for (var n=0; n<ns.length; n++) ns[n].y= (ns[n].y+k) % 88;
// 		for (var n=0; n<ns.length; n++) ns[n].y= (ns[n].y+9+k) % 12;
// 		for (var s=0; s<scaleDict.length-1; s++) 
// 		if (s!=91)  // excluding chromatic
// //		if (s!=91 && s!=88)  // excluding chromatic & composite blues
// 		{	var inScaleCount=0, inScaleDuration=0;
// 			for (var n=0; n<ns.length; n++){
// //				if (scaleDict[s].mask[(ns[n].y+9) % 12]=='1') {
// 				if (scaleDict[s].mask[ns[n].y]=='1') {
// 					inScaleCount++;
// 					inScaleDuration+=ns[n].d;
// 				};
// 			};
// 			if (inScaleDuration / totalDuration == maxInScale) {
// 					maxRes.push({maxK:(12-k) % 12, maxKN: keyNames[(12-k) % 12],
...
// n starting from 1 up to the length of the chord key count (e.g. 7 for major / minor and 5 for pentatonic)
function getIndexOfNthKeyFromScale(s, n){
	var cc=0;
	for (var i=0; i<12; i++) if (s.mask[i]==1) cc++;
	n = n % cc;
	cc=0; 
	for (var i=0; i<12; i++) {
		if (s.mask[i]==1) {
...
// to get the scale key from a chromatic key
// input: scale, key in the chromatic scale (eg. ionian /4)
// output: serial number of a key in the scale (eg. 3 - mi)
function getScaleKeyFromChromaticKey(s, n){
	if (s.mask[n]==0) return 0;
	var cc=0;
	for (var i=0; i<n+1; i++) {
		if (s.mask[i]==1) cc++;
	};
...
Theory.getSlimChordsByKeyScale=getSlimChordsByKeyScale;
